...
class Transaction(db.Model):
    ...
    def __repr__(self):
        currency_code = self.currency_rel.code if self.currency_rel else 'N/A'
        return f"<Transaction {self.id} ({self.amount} {currency_code}) for Purchase {self.purchase_id}>"
    ...
    def create(cls, data: Dict) -> 'Transaction':
        ...
        try:
            ...
            # Obter o ID da moeda - assumindo que 'currency_code' é passado em 'data'
            currency_code = data.get("currency_code", "BRL").upper() # Default para BRL se não fornecido
            currency_obj = Currency.get_by_code(currency_code)
            if not currency_obj:
